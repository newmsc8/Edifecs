...
for(i in c(grep("^CM_",colnames(df)),grep("^REVCD_",colnames(df)),grep("^DXCCS_",colnames(df)),grep("^PRCCS_",colnames(df)))) {
	...
	cbind(df.lsa,df[,i])
	colnames(df.lsa)[ncol(df.lsa)]<-colnames(df)[i]
	df.filtered[,i]<-NULL
}
...
df.filtered<-cbind(df.filtered,df.lsa)

#for(i in 1:length(colnames(df.lsa))) {
#	for(j in c(grep(colnames(df.lsa)[i],colnames(df)))) {
#		df[,j]<-df.lsa[,i]
#	}
#}
